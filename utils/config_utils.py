...
def overwrite_values(config, args):
    ...
    for key, value in zip(keys, values):
        if '.' in key:
            # Nested key - use original behavior
            parent, last_key = get_parent_dict(config, key)
            ori_value = parent[last_key]
            parent[last_key] = type(ori_value)(value)
        else:
            # Top-level key - might not exist (e.g., load_ckpt)
            if key in config:
                # Key exists, preserve original type
                ori_value = config[key]
                config[key] = type(ori_value)(value)
            else:
                # Key doesn't exist, add it as string
                config[key] = value
    return config
