...
def parse():
    ...
    parser.add_argument('--max_peptide_length', type=int, default=30,
                        help='Maximum peptide sequence length to process (default: 30 amino acids)')
    return parser.parse_args()


def process_iterator(items, pdb_dir, pdb_out_dir, pocket_th, max_peptide_length=30):
    ...
    for cnt, pdb_id in enumerate(items):
        ...
        # Remove debug statement that was causing issues
        non_standard = 0
        try:
        ...
        print(f'{pdb_id} {lig_seq}')
        
        # Filter based on peptide sequence length (amino acids)
        if len(lig_seq) > max_peptide_length:
            skipped_long += 1
            if skipped_long <= 10:  # Show first 10 skipped peptides
                print_log(f'Skipping {pdb_id}: peptide sequence length {len(lig_seq)} > {max_peptide_length} aa')
            elif skipped_long == 11:
                print_log(f'... (showing only first 10 skipped peptides)')
            continue
    ...
    print_log(f'Processing complete:')
    print_log(f'  Processed: {processed_count} peptides (≤{max_peptide_length} aa)')
    print_log(f'  Skipped:   {skipped_long} peptides (>{max_peptide_length} aa)')
    print_log(f'  Total:     {processed_count + skipped_long} peptides')
...
def main(args):
    ...
    print_log(f'Maximum peptide sequence length: {args.max_peptide_length} amino acids')
