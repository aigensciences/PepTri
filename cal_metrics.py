...
from evaluation.tmscore import compute_tmscore
from evaluation.local_metrics import compute_peptide_metrics
...
def cal_metrics(items):
    ...
    for item in items:
        ...
        # TM-score calculation (for comparison, but local metrics are better for peptides)
        try:
            ref_seq = ''.join([block.abrv for block in ref_pep_blocks])
            gen_seq = ''.join([block.abrv for block in gen_pep_blocks])
            tm_score = compute_tmscore(
                ref_coords=ref_ca_x[ca_mask], 
                pred_coords=gen_ca_x[ca_mask],
                ref_seq=ref_seq,
                pred_seq=gen_seq,
                method='auto'
            )
            results['TM-score'].append(tm_score)
            if struct_only:
                results['TM-score>=0.5'].append(1 if tm_score >= 0.5 else 0)
                results['TM-score>=0.7'].append(1 if tm_score >= 0.7 else 0)
                results['TM-score>=0.9'].append(1 if tm_score >= 0.9 else 0)
        except Exception as e:
            print(f"Warning: TM-score calculation failed for {item['id']}: {e}")
            results['TM-score'].append(0.0)
            if struct_only:
                results['TM-score>=0.5'].append(0)
                results['TM-score>=0.7'].append(0)
                results['TM-score>=0.9'].append(0)

        # Local metrics (BETTER for peptides - GDT_TS, contact overlap, local RMSD)
        try:
            local_metrics = compute_peptide_metrics(
                ref_coords=ref_ca_x[ca_mask], 
                pred_coords=gen_ca_x[ca_mask],
                ref_seq=ref_seq,
                pred_seq=gen_seq
            ...
            # GDT_TS scores (better than TM-score for short peptides)
            results['GDT_TS'].append(local_metrics.get('GDT_TS', 0.0))
            results['GDT_1A'].append(local_metrics.get('GDT_1.0', 0.0))
            results['GDT_2A'].append(local_metrics.get('GDT_2.0', 0.0))
            results['GDT_4A'].append(local_metrics.get('GDT_4.0', 0.0))
            results['GDT_8A'].append(local_metrics.get('GDT_8.0', 0.0))
            
            # Contact overlap (measures local structural similarity)
            results['Contact_Overlap'].append(local_metrics.get('contact_overlap', 0.0))
            results['Contact_Precision'].append(local_metrics.get('contact_precision', 0.0))
            results['Contact_Recall'].append(local_metrics.get('contact_recall', 0.0))
            results['Contact_F1'].append(local_metrics.get('contact_f1', 0.0))
            
            # Local RMSD (sliding window analysis)
            results['Local_RMSD_Mean'].append(local_metrics.get('local_rmsd_mean', 999.0))
            results['Local_RMSD_Min'].append(local_metrics.get('local_rmsd_min', 999.0))
            results['Local_RMSD_Max'].append(local_metrics.get('local_rmsd_max', 999.0))
            
            if struct_only:
                # GDT_TS thresholds (more meaningful for peptides than TM-score)
                results['GDT_TS>=0.5'].append(1 if local_metrics.get('GDT_TS', 0.0) >= 0.5 else 0)
                results['GDT_TS>=0.7'].append(1 if local_metrics.get('GDT_TS', 0.0) >= 0.7 else 0)
                results['GDT_TS>=0.9'].append(1 if local_metrics.get('GDT_TS', 0.0) >= 0.9 else 0)
                
                # Contact overlap thresholds  
                results['Contact_Overlap>=0.3'].append(1 if local_metrics.get('contact_overlap', 0.0) >= 0.3 else 0)
                results['Contact_Overlap>=0.5'].append(1 if local_metrics.get('contact_overlap', 0.0) >= 0.5 else 0)
                results['Contact_Overlap>=0.7'].append(1 if local_metrics.get('contact_overlap', 0.0) >= 0.7 else 0)
                
        except Exception as e:
            print(f"Warning: Local metrics calculation failed for {item['id']}: {e}")
            # Provide default values
            defaults = {
                'GDT_TS': 0.0, 'GDT_1A': 0.0, 'GDT_2A': 0.0, 'GDT_4A': 0.0, 'GDT_8A': 0.0,
                'Contact_Overlap': 0.0, 'Contact_Precision': 0.0, 'Contact_Recall': 0.0, 'Contact_F1': 0.0,
                'Local_RMSD_Mean': 999.0, 'Local_RMSD_Min': 999.0, 'Local_RMSD_Max': 999.0
            }
            for key, default_val in defaults.items():
                results[key].append(default_val)
            
            if struct_only:
                for threshold_key in ['GDT_TS>=0.5', 'GDT_TS>=0.7', 'GDT_TS>=0.9',
                                    'Contact_Overlap>=0.3', 'Contact_Overlap>=0.5', 'Contact_Overlap>=0.7']:
                    results[threshold_key].append(0)
...
def main(args):
    ...
    summary_lines = []
    summary_lines.append('Point-wise evaluation results:')
    ...
    for name in metrics[0]:
        ...
        if isinstance(vals[0], dict):
            ...
            if '>=' in name or '<=' in name:  # percentage
                line = f'{name}: {sum(aggr_vals) / len(aggr_vals)}'
                print(line)
                summary_lines.append(line)
            else:
                if 'RMSD' in name:
                    line = f'{name}(median): {statistics.median(aggr_vals)}'  # unbounded, some extreme values will affect the mean but not the median
                    print(line)
                    summary_lines.append(line)
                else:
                    line = f'{name}(mean): {sum(aggr_vals) / len(aggr_vals)}'
                    print(line)
                    summary_lines.append(line)
                ...
                low_line = f'\tlowest: {aggr_vals[lowest_i]}, id: {ids[lowest_i]}'
                high_line = f'\thighest: {aggr_vals[highest_i]}, id: {ids[highest_i]}'
                print(low_line)
                print(high_line)
                summary_lines.append(low_line)
                summary_lines.append(high_line)
        else:
            line = f'{name} (mean): {sum(vals) / len(vals)}'
            print(line)
            summary_lines.append(line)
            lowest_i = min([i for i in range(len(vals))], key=lambda i: vals[i])
            highest_i = max([i for i in range(len(vals))], key=lambda i: vals[i])
            low_line = f'\tlowest: {vals[lowest_i]}, id: {ids[lowest_i]}'
            high_line = f'\thighest: {vals[highest_i]}, id: {ids[highest_i]}'
            print(low_line)
            print(high_line)
            summary_lines.append(low_line)
            summary_lines.append(high_line)
    ...
    summary_path = os.path.join(os.path.dirname(args.results), 'evaluation_summary.txt')
