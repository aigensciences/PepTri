...
def main(args):
    ...
    # Print to console
    print(f'median: {median_val}, mean: {mean_val}')
    print(f'mean success rate: {success_rate}')
    
    # Save summary to file
    summary_path = os.path.join(os.path.dirname(args.out_path), 'dG_summary.txt')
    with open(summary_path, 'w') as f:
        f.write(f'dG Analysis Summary\n')
        f.write(f'==================\n\n')
        f.write(f'Total samples analyzed: {len(results)}\n')
        f.write(f'Median dG: {median_val:.4f}\n')
        f.write(f'Mean dG: {mean_val:.4f}\n')
        f.write(f'Mean success rate: {success_rate:.4f}\n\n')
        f.write(f'Individual results:\n')
        for _id in sorted(results.keys()):
            f.write(f'{_id}: min_dG={results[_id]["min"]:.4f}, success_rate={results[_id]["success rate"]:.4f}\n')
    
    print(f'\nDetailed results saved to: {args.out_path}')
