...
def design(mode, ckpt, gpu, pdbs, epitope_defs, n_samples, out_dir,
    ...
    model = load_checkpoint_with_module_mapping(ckpt, map_location='cpu')
    model.to(device)
    model.eval()
...
def parse():
    ...
    parser.add_argument('--ckpt', type=str, required=False, help='Path to the checkpoint of the trained model')
...
if __name__ == '__main__':
    ...
    ckpt = args.ckpt or os.path.join(proj_dir, 'checkpoints', 'fixseq.ckpt' if args.mode == 'struct_pred' else 'codesign.ckpt')
